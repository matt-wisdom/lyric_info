...
class LyricsDotComScraper():
    ...
    decade = [x for x in range(1930, 2020, 10)]
    albums = "#content-body > div:nth-child(1) > div:nth-child(4) > p:nth-child(3) > button:nth-child(1)"
    ...
    def parse_params(**kwargs):
        """
            This method takes in a dictionary of parameters to be used in the url and their value(s).
             It can take multiple values for each parameters i  and generates all the various
             parameters combination..table > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(1) > div:nth-child(2) > div:nth-child(2) > a:nth-child(1)

        Allowed parameters are
        'stype': This is specifies the type of match for the search it takes values 2 for exact match
                and 1 for Exact match within title. It defaults to within lyrics if not specified.
        'gender': Can either be 'm' for male, 'f' for female or 'g' for group. Defaults to all.
                it can also take a list or tuple of genders for multiple matches
        'qtype': Can be 3 to only show albums that match the search or 2 to show artists that match the search
                 it can also take a list or tuple of both as values for multiple matches
        'genre': Can be any of the items in the genres class variable.    
            it can also take a list or tuple of multiple genres as values for multiple matches
        'decade':Takes any (or a tuple(list) of multiple values) of the values specified in the decade class
                variable
        "year": It can be a year of a tuple or list of years between 1900 and whatever year this is.
        "style": It can be a value of a tuple or list of values in the style class variable.
        ''
        """
        ...
        params = [{} for i in range(noofparams)] # List of parameter dictionaries for multiple searches
        for i in kwargs:
            if len(kwargs[i]):
    ...
    def search(self, params, search_path='serp.php'):
        ...
        for i in params:
            ...
            try:
                ...
                message = f"Search Results: {lyrics.text} lyrics, {artists.text} artists, {albums.text} albums for search term {search_term.text}"
                print(message)
            except:
    ...
    @staticmethod
    def extract_full_lyrics(full_lyric_url, title, download=False, filename="[[auto_generate]]"):
        """
        ...
        if download:
            if filename == "[[auto_generate]]":
                filename = title.replace(" ", "_") + "-" + str(random.randrange(10000000, 99999999)) +'-lyrics.txt'
    ...
    def extract_lyrics_remove_tags(content):
        remove_tags_regex = s=re.compile(r'(<.*>)(?=(\w+<))') # Regex with findall splits tag elements into_opening, content, closing_tag
        lyricsobj = BeautifulSoup(content, 'lxml')
        try:
            open("gg.html", "w").write(lyricsbody)
        ...
        for j in self.params_bs4objs:
            if j[0].get('qtype') == 1:
                ...
                for i in lyrics_select:
                    ...
                    artist, artist_link = self.extract_data(i, '.lyric-meta-album-artist')
                    year, year_link = self.extract_data(i, '.lyric-meta-album-year')
                    album, album_url = self.extract_data(i, ".lyric-meta-album")
                    url, snippet = LyricsDotComScraper.extract_fulllyrics_url_with_snippet(i)
                    item['title'] = title
    ...
    def extract_album_content(html):
